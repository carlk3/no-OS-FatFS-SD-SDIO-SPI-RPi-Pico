...
#define STATE_P sd_card_p->sdio_if.state_p
#define SDIO_PIO sd_card_p->sdio_if.SDIO_PIO
#define SDIO_CMD_SM STATE_P->SDIO_CMD_SM
#define SDIO_DATA_SM STATE_P->SDIO_DATA_SM
#define SDIO_DMA_CH STATE_P->SDIO_DMA_CH
#define SDIO_DMA_CHB STATE_P->SDIO_DMA_CHB
        ...
        if (absolute_time_diff_us(get_absolute_time(), timeout_time) <= 0)
        {
            if (command != 8) // Don't log for missing SD card
            {
                azdbg("Timeout waiting for response in rp2040_sdio_command_R1(", (int)command, "), ",
                    "PIO PC: ", (int)pio_sm_get_pc(SDIO_PIO, SDIO_CMD_SM) - (int)STATE_P->pio_cmd_clk_offset,
            ...
            pio_sm_clear_fifos(SDIO_PIO, SDIO_CMD_SM);
            pio_sm_exec(SDIO_PIO, SDIO_CMD_SM, pio_encode_jmp(STATE_P->pio_cmd_clk_offset));
...
sdio_status_t rp2040_sdio_rx_start(sd_card_t *sd_card_p, uint8_t *buffer, uint32_t num_blocks)
{
    ...
    STATE_P->transfer_state = SDIO_RX;
    STATE_P->transfer_timeout_time = make_timeout_time_ms(1000);
    STATE_P->data_buf = (uint32_t*)buffer;
    STATE_P->blocks_done = 0;
    STATE_P->total_blocks = num_blocks;
    STATE_P->blocks_checksumed = 0;
    STATE_P->checksum_errors = 0;

    // Create DMA block descriptors to store each block of 512 bytes of data to buffer
    // and then 8 bytes to STATE_P->received_checksums.
    for (uint32_t i = 0; i < num_blocks; i++)
    {
        STATE_P->dma_blocks[i * 2].write_addr = buffer + i * SDIO_BLOCK_SIZE;
        STATE_P->dma_blocks[i * 2].transfer_count = SDIO_BLOCK_SIZE / sizeof(uint32_t);

        STATE_P->dma_blocks[i * 2 + 1].write_addr = &STATE_P->received_checksums[i];
        STATE_P->dma_blocks[i * 2 + 1].transfer_count = 2;
    }
    STATE_P->dma_blocks[num_blocks * 2].write_addr = 0;
    STATE_P->dma_blocks[num_blocks * 2].transfer_count = 0;
    ...
    dma_channel_configure(SDIO_DMA_CHB, &dmacfg, &dma_hw->ch[SDIO_DMA_CH].al1_write_addr,
        STATE_P->dma_blocks, 2, false);

    // Initialize PIO state machine
    pio_sm_init(SDIO_PIO, SDIO_DATA_SM, STATE_P->pio_data_rx_offset, &STATE_P->pio_cfg_data_rx);
...
{
    while (STATE_P->blocks_checksumed < STATE_P->blocks_done && maxcount-- > 0)
    {
        // Calculate checksum from received data
        int blockidx = STATE_P->blocks_checksumed++;
        uint64_t checksum = sdio_crc16_4bit_checksum(STATE_P->data_buf + blockidx * SDIO_WORDS_PER_BLOCK,
                                                     SDIO_WORDS_PER_BLOCK);

        // Convert received checksum to little-endian format
        uint32_t top = __builtin_bswap32(STATE_P->received_checksums[blockidx].top);
        uint32_t bottom = __builtin_bswap32(STATE_P->received_checksums[blockidx].bottom);
        uint64_t expected = ((uint64_t)top << 32) | bottom;

        if (checksum != expected)
        {
            STATE_P->checksum_errors++;
            if (STATE_P->checksum_errors == 1)
            {
                ...
                dump_bytes(SDIO_WORDS_PER_BLOCK, (uint8_t *)STATE_P->data_buf + blockidx * SDIO_WORDS_PER_BLOCK);
            }
        }
    }
}
...
sdio_status_t rp2040_sdio_rx_poll(sd_card_t *sd_card_p, uint32_t *bytes_complete)
{
    ...
    if (STATE_P->blocks_done >= STATE_P->total_blocks)
    {
        STATE_P->transfer_state = SDIO_IDLE;
    }
    else
    {
        ...
        uint32_t dma_ctrl_block_count = (dma_hw->ch[SDIO_DMA_CHB].read_addr - (uint32_t)&STATE_P->dma_blocks);
        dma_ctrl_block_count /= sizeof(STATE_P->dma_blocks[0]);

        // Compute how many complete 512 byte SDIO blocks have been transferred
        // When transfer ends, dma_ctrl_block_count == STATE_P->total_blocks * 2 + 1
        STATE_P->blocks_done = (dma_ctrl_block_count - 1) / 2;
    ...
    if (bytes_complete)
    {
        *bytes_complete = STATE_P->blocks_done * SDIO_BLOCK_SIZE;
    }

    if (STATE_P->transfer_state == SDIO_IDLE)
    {
        // Verify all remaining checksums.
        sdio_verify_rx_checksums(sd_card_p, STATE_P->total_blocks);

        if (STATE_P->checksum_errors == 0)
            return SDIO_OK;
        else
            return SDIO_ERR_DATA_CRC;
    }
    else if ((absolute_time_diff_us(get_absolute_time(), STATE_P->transfer_timeout_time) < 0))
    {
        azdbg("rp2040_sdio_rx_poll() timeout, "
            "PIO PC: ", (int)pio_sm_get_pc(SDIO_PIO, SDIO_DATA_SM) - (int)STATE_P->pio_data_rx_offset,
...
{
    // Initialize PIO
    pio_sm_init(SDIO_PIO, SDIO_DATA_SM, STATE_P->pio_data_tx_offset, &STATE_P->pio_cfg_data_tx);
    ...
    dma_channel_configure(SDIO_DMA_CH, &dmacfg,
        &SDIO_PIO->txf[SDIO_DATA_SM], STATE_P->data_buf + STATE_P->blocks_done * SDIO_WORDS_PER_BLOCK,
        SDIO_WORDS_PER_BLOCK, false);

    // Prepare second DMA channel to send the CRC and block end marker
    uint64_t crc = STATE_P->next_wr_block_checksum;
    STATE_P->end_token_buf[0] = (uint32_t)(crc >> 32);
    STATE_P->end_token_buf[1] = (uint32_t)(crc >>  0);
    STATE_P->end_token_buf[2] = 0xFFFFFFFF;
    channel_config_set_bswap(&dmacfg, false);
    dma_channel_configure(SDIO_DMA_CHB, &dmacfg,
        &SDIO_PIO->txf[SDIO_DATA_SM], STATE_P->end_token_buf, 3, false);
...
{
    assert (STATE_P->blocks_done < STATE_P->total_blocks && STATE_P->blocks_checksumed < STATE_P->total_blocks);
    int blockidx = STATE_P->blocks_checksumed++;
    STATE_P->next_wr_block_checksum = sdio_crc16_4bit_checksum(STATE_P->data_buf + blockidx * SDIO_WORDS_PER_BLOCK,
                                                             SDIO_WORDS_PER_BLOCK);
}
...
sdio_status_t rp2040_sdio_tx_start(sd_card_t *sd_card_p, const uint8_t *buffer, uint32_t num_blocks)
{
    ...
    STATE_P->transfer_state = SDIO_TX;
    STATE_P->transfer_timeout_time = make_timeout_time_ms(2000); // CK3: doubled timeout
    STATE_P->data_buf = (uint32_t*)buffer;
    STATE_P->blocks_done = 0;
    STATE_P->total_blocks = num_blocks;
    STATE_P->blocks_checksumed = 0;
    ...
    if (STATE_P->blocks_checksumed < STATE_P->total_blocks)
    {
...
static void sub_rp2040_sdio_tx_irq(sd_card_t *sd_card_p) {
    if (STATE_P->transfer_state == SDIO_TX)
    {
        if (!dma_channel_is_busy(SDIO_DMA_CH) && !dma_channel_is_busy(SDIO_DMA_CHB))
        {
            ...
            STATE_P->transfer_state = SDIO_TX_WAIT_IDLE;
            if (!pio_sm_is_rx_fifo_empty(SDIO_PIO, SDIO_DATA_SM))
            {
                // Card is already idle
                STATE_P->card_response = pio_sm_get(SDIO_PIO, SDIO_DATA_SM);
            }
            else
            {
                ...
                channel_config_set_dreq(&dmacfg, pio_get_dreq(SDIO_PIO, SDIO_DATA_SM, false));
                dma_channel_configure(SDIO_DMA_CHB, &dmacfg,
                    &STATE_P->card_response, &SDIO_PIO->rxf[SDIO_DATA_SM], 1, true);
            }
        }
    }
    
    if (STATE_P->transfer_state == SDIO_TX_WAIT_IDLE)
    {
        if (!dma_channel_is_busy(SDIO_DMA_CHB))
        {
            STATE_P->wr_status = check_sdio_write_response(STATE_P->card_response);

            if (STATE_P->wr_status != SDIO_OK)
            {
                rp2040_sdio_stop(sd_card_p);
                return;
            }
...
sdio_status_t rp2040_sdio_tx_poll(sd_card_t *sd_card_p, uint32_t *bytes_complete)
{
    ...
    {
        rp2040_sdio_stop(sd_card_p);
        return STATE_P->wr_status;
    }
    // else if ((uint32_t)(millis() - STATE_P->transfer_start_time) > 1000)
    else if ((absolute_time_diff_us(get_absolute_time(), STATE_P->transfer_timeout_time) <= 0))
    {
        azdbg("rp2040_sdio_tx_poll() timeout, "
            "PIO PC: ", (int)pio_sm_get_pc(SDIO_PIO, SDIO_DATA_SM) - (int)STATE_P->pio_data_tx_offset,
            " RXF: ", (int)pio_sm_get_rx_fifo_level(SDIO_PIO, SDIO_DATA_SM),
            " TXF: ", (int)pio_sm_get_tx_fifo_level(SDIO_PIO, SDIO_DATA_SM),
            " DMA CNT: ", dma_hw->ch[SDIO_DMA_CH].al2_transfer_count);
        rp2040_sdio_stop(sd_card_p);
...
bool rp2040_sdio_init(sd_card_t *sd_card_p, float clk_div) {
    ...
    if (!STATE_P->resources_claimed) {
        assert(!STATE_P);
        STATE_P = malloc(sizeof(sd_sdio_state_t));
        if (!STATE_P) {
            printf("malloc failed!\n");
            return false;
        }
        memset(STATE_P, 0, sizeof(sd_sdio_state_t));
        ...
        STATE_P->resources_claimed = true;
    }
    ...
    STATE_P->pio_cmd_clk_offset = pio_add_program(SDIO_PIO, &sdio_cmd_clk_program);
    pio_sm_config cfg = sdio_cmd_clk_program_get_default_config(STATE_P->pio_cmd_clk_offset);
    ...
    pio_sm_init(SDIO_PIO, SDIO_CMD_SM, STATE_P->pio_cmd_clk_offset, &cfg);
    pio_sm_set_consecutive_pindirs(SDIO_PIO, SDIO_CMD_SM, SDIO_CLK, 1, true);
    pio_sm_set_enabled(SDIO_PIO, SDIO_CMD_SM, true);

    // Data reception program
    STATE_P->pio_data_rx_offset = pio_add_program(SDIO_PIO, &sdio_data_rx_program);
    STATE_P->pio_cfg_data_rx = sdio_data_rx_program_get_default_config(STATE_P->pio_data_rx_offset);
    sm_config_set_in_pins(&STATE_P->pio_cfg_data_rx, SDIO_D0);
    sm_config_set_in_shift(&STATE_P->pio_cfg_data_rx, false, true, 32);
    sm_config_set_out_shift(&STATE_P->pio_cfg_data_rx, false, true, 32);
    sm_config_set_clkdiv(&STATE_P->pio_cfg_data_rx, clk_div);

    // Data transmission program
    STATE_P->pio_data_tx_offset = pio_add_program(SDIO_PIO, &sdio_data_tx_program);
    STATE_P->pio_cfg_data_tx = sdio_data_tx_program_get_default_config(STATE_P->pio_data_tx_offset);
    sm_config_set_in_pins(&STATE_P->pio_cfg_data_tx, SDIO_D0);
    sm_config_set_set_pins(&STATE_P->pio_cfg_data_tx, SDIO_D0, 4);
    sm_config_set_out_pins(&STATE_P->pio_cfg_data_tx, SDIO_D0, 4);
    sm_config_set_in_shift(&STATE_P->pio_cfg_data_tx, false, false, 32);
    sm_config_set_out_shift(&STATE_P->pio_cfg_data_tx, false, true, 32);
    sm_config_set_clkdiv(&STATE_P->pio_cfg_data_tx, clk_div);
